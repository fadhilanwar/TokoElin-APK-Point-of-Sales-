              ...
              <div class="card-body">
                <div class="border-bottom py-3 mb-3">
                    <button data-bs-toggle="modal" data-bs-target="#cartPasok" class="btn btn-success w-25">
                       Lihat Keranjang Pasok
                    </button>
                   
                  </div>
                  <form action="/pasokbarang/{{ $post->id }}/editQTY" method="post">
                    @csrf
                {{-- <div class="row">
                ...
                <div class="row mb-4 mt-4">
                    ...
                    <div class="col md-4">
                      {{-- HIDDEN FIELD START --}}
                      {{-- ID --}}
                      <input value="{{ $list_keranjang->id }}" type="hidden" class="form-control txt" name="id">
                      {{-- Harga Grosir --}}
                      <input value="{{ $list_keranjang->harga_grosir }}" type="hidden" class="form-control txt" name="harga_grosir">
                      {{-- HIDDEN FIELD END END --}}
                       <label class="col-form-label">Kode / ID Barang :</label>
                        <input value="{{ $list_keranjang->id_barang }}" type="text" class="form-control txt" name="id_barang">

                      <label class="col-form-label">Nama Barang :</label>
                        <input value="{{ $list_keranjang->nama_barang }}" type="text" class="form-control txt" name="nama_barang">
                    </div>
                    <div class="col md-4">
                        <label for="id_pasok" class="col-form-label">Qty :</label>
                        
                    <input value="{{ $list_keranjang->qty }}" type="number" class="form-control txt" name="qty" required>
                    ...
                    <label for="id_pasok" class="col-form-label pb-4 mt-1">
                     ...
                     <button type="submit" style="width: 100%" class="btn btn-info">Ubah QTY Belanja</button>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div id="table-default" class="table-responsive">
                    ...
                    @endphp
                    <tbody class="table-tbody">
                        @foreach ($list_keranjang_all as $c)
                        <tr>
                            <th>{{ $no++ }}</th>
                            <td>{{ $c->id_barang }}</td>
                            <td>{{ $c->nama_barang }}</td>
                            ...
                            <td class="d-flex">
                                <form action="/pasokbarang/{{ $c->id }}/deleteFromCart" method="get">
                                    <button class="btn btn-danger mr-2"><i class="fa fa-trash"></i>Hapus List</button>                                
                                </form>
